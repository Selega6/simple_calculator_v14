    ...
    const std::vector<gv>& args)
{
    if(args.size() != f.params.size())
        error("wrong number of arguments for function " + f.name);
    ...
    const std::vector<gv>& args)
{
    ...
    // Añadimos ';' porque el parser lo exige
    processed += ";";

    std::istringstream iss(processed);

    // Guardamos el buffer de cin
    std::streambuf* oldbuf = std::cin.rdbuf(iss.rdbuf());

    // 1. BACKUP del Token_stream global
    Token_stream ts_backup = ts;
    ts = Token_stream(); // 2. Reiniciar el Token_stream para la nueva entrada

    gv result = expression();

    // 3. Restaurar cin y el Token_stream global
...
gv primary()
{
    ...
    // name → variable OR generic function
    if (t.kind == Token::name_token)
    {
        string fname = t.name;
        Token t2 = ts.get();

        if(t2.is_symbol('(')) // CALL
        {
            vector<gv> args;
            Token nxt = ts.get();
        ...
        if (names.count(fname))
          return names[fname].value;

      error("get: undefined name ", fname);
    ...
    if(t.is_symbol('('))
    {
        gv v = expression();
        Token t2 = ts.get();
        if(!t2.is_symbol(')')) error("')' expected");
        return v;
    }
    ...
    if(t.is_symbol('{'))
    {
        ...
        if(t2.is_symbol('{'))
        {
            ...
            gv m = columns();
            Token close = ts.get();
            if(!close.is_symbol('}')) error("'}' expected");
            return m;
        }
        ...
        Token close = ts.get();
        if(!close.is_symbol('}')) error("'}' expected");
        return gv(typename gv::matrix_t::value_t(row));
    }

    if(t.is_symbol('-')) return -primary();
    if(t.is_symbol('+')) return primary();
    if(t.is_symbol('~')) return ~primary();
        ...
        if(t2.is_symbol('('))
        {
            ...
            while(true)
            {
                ...
                if(tk.kind == Token::number)
                {
                    ...
                    body << tk.name;
                else if(tk.kind == Token::char_token)
                    body << tk.symbol;
                else
